...
class PIDaxis():
    def __init__(self, kp, ki, kd, i_range=None, d_range=None, control_range=(1000, 2000), midpoint=1500, smoothing=True):
        ...
        self.debug = False  # Enable/disable debug printing
    ...
    def adaptive_height_step(self, err, time_elapsed):
        ...
        if self.takeoff_phase and current_height >= TARGET_HEIGHT * 0.9 and \
           (current_time - self.takeoff_start_time) > 2.0:
            # Only exit takeoff if we're moving slowly
            if abs(self.vertical_velocity) < 0.1:
                self.takeoff_phase = False
                if self.debug:
                    print("Exiting takeoff phase at height %.2fm with velocity %.2fm/s" % 
        ...
        if self.debug:
            print("Height: %.2fm, Target: %.2fm, V_speed: %.2fm/s, Takeoff: %s, Stage: %d, OnGround: %s" % 
        ...
        if current_height > TARGET_HEIGHT * 1.4:  # Снижено с 1.5 до 1.4
            if self.debug:
        ...
        if self.vertical_velocity < -MAX_VERTICAL_SPEED * 1.1:  # Снижено с 1.2 до 1.1
            if self.debug:
                print("EMERGENCY THROTTLE: falling too fast! %.2f m/s" % self.vertical_velocity)
            return HOVER_THROTTLE + 10  # Снижено с +15 до +10
        ...
        if current_height > TARGET_HEIGHT:
            ...
            if height_diff > 0.2:  # Снижено с 0.3 до 0.2
                ...
                base_throttle = HOVER_THROTTLE - 25  # Уменьшено с -30 до -25 с учетом нового HOVER_THROTTLE
                gain = 0.5   # Уменьшено с 0.55
            elif height_diff > 0.08:  # Снижено с 0.1 до 0.08
                base_throttle = HOVER_THROTTLE - 15  # Уменьшено с -18 с учетом нового HOVER_THROTTLE
                gain = 0.55  # Уменьшено с 0.6
            else: 
                # Close to target - gentle descent
                base_throttle = HOVER_THROTTLE - 7   # Уменьшено с -8 с учетом нового HOVER_THROTTLE
                gain = 0.65  # Уменьшено с 0.7
                
            # Reduce descent rate if already descending quickly
            if self.vertical_velocity < -0.12:  # Снижено с -0.15 до -0.12
            ...
            if self.takeoff_phase:
                ...
                if height_diff > 0.2:  # Снижено с 0.3 до 0.2
                    ...
                    base_throttle = HOVER_THROTTLE + 15  # Снижено с +20 до +15 с учетом нового HOVER_THROTTLE
                    gain = 0.8  # Снижено с 0.85 до 0.8
                elif height_diff > 0.08:  # Снижено с 0.1 до 0.08
                    # Moderately below target
                    base_throttle = HOVER_THROTTLE + 10  # Снижено с +12 до +10 с учетом нового HOVER_THROTTLE
                    gain = 0.7  # Снижено с 0.75 до 0.7
                elif height_diff > 0.04:  # Снижено с 0.05 до 0.04
                    # Slightly below target
                    base_throttle = HOVER_THROTTLE + 5   # Снижено с +6 до +5 с учетом нового HOVER_THROTTLE
                    gain = 0.6  # Снижено с 0.65 до 0.6
                else:
                    # Very close to target
                    base_throttle = HOVER_THROTTLE
                    gain = 0.55  # Снижено с 0.6 до 0.55
        ...
        max_throttle_change = 8  # Максимальное изменение за один шаг (уменьшено с 10)
        if self.takeoff_phase:
            # Во время взлета еще более плавное изменение
            max_throttle_change = 4  # Уменьшено с 5
        ...
        if normalized_throttle < 0.5:
            normalized_throttle = 0.5 * math.pow(normalized_throttle * 2, 1.05)  # Снижено с 1.1 до 1.05
        else:
            normalized_throttle = 0.5 + 0.5 * (1.0 - math.pow(2.0 - normalized_throttle * 2, 1.05))  # Снижено с 1.1 до 1.05
        ...
        if self.debug:
            print("Throttle: %d, Error: %.2f, V: %.2f m/s, P: %.2f, I: %.2f, D: %.2f" % 
...
class PID:
    def __init__(self,
                 ...
                 throttle=PIDaxis(1, 0, 1, i_range=(-400, 400), control_range=(1100, 1600),
    ...
    def step(self, error, cmd_yaw_velocity=0):
        ...
        # Return commands in order [ROLL, PITCH, THROTTLE, YAW]
