...
class PIDController(object):
    ...
    def step(self):
        ...
        # Handle takeoff stabilization if active
        if self.stabilizing_takeoff:
            stabilization_complete = self.takeoff_stabilization()
            if not stabilization_complete:
                # During stabilization, focus only on maintaining height
                # Set horizontal errors to zero to prevent drift
                self.pid_error.x = 0
                self.pid_error.y = 0
                # Keep only the z (height) component for stabilization
